...
const key = Symbol() as InjectionKey<Transport>;
...
/**
 * Use this helper to get the default transport that's currently attached to the Vue App.
 */
export const useTransport = () => inject(key) ?? fallbackTransport;

/**
 * `ConnectTransport` plugin is the main mechanism by which Connect-Query keeps track of the `Transport` used by your application.
 *
 ...
 * import { ConnectTransport } from 'connect-vue-query'
 *
...
export const ConnectTransport = {
  install(app: App, option: {
    ...
    app.provide(key, option.transport);
  },
};
